...
namespace Reports
{
    public class Отчет : Stimulsoft.Report.StiReport
    {
        ...
        private void InitializeComponent()
        {
            ...
            this.Dictionary.Databases.Add(new Stimulsoft.Report.Dictionary.StiSqlDatabase("FitrstTask", "FitrstTask", "Integrated Security=True;Data Source=DESKTOP-8LKEMKN;Initial Catalog=ServiceRegister;Password=;User ID=", false));
        ...
        {
            this.Test.SqlCommand = "IF @CompanyId > 0 AND @ServiceId > 0 AND @EmployeeId > 0\r\n(SELECT  Services.Name as ServiceName, Services.Price as ServicePrice, Employee.FullName as EmployeeName, Companies.Name as CompanyName, \r\nCOUNT(*) OVER() AS TotalRows\tFROM ServiceHistory \r\n\tINNER JOIN Services ON Services.Id = ServiceHistory.ServiceId\r\nINNER JOIN Employee ON Employee.Id = ServiceHistory.EmployeeId\r\nINNER JOIN Companies ON Companies.Id = ServiceHistory.CompanyId\r\nWHERE CONVERT(date, DateOfCreate) >= CONVERT(date, @DateBegin) AND CONVERT(date, DateOfCreate) <= CONVERT(date, @DateEnd) \r\nAND ServiceHistory.CompanyId = @CompanyId AND ServiceId = @ServiceId AND EmployeeId = EmployeeId)\r\n\r\nELSE IF @CompanyId > 0 AND @ServiceId > 0\r\n(SELECT  Services.Name as ServiceName, Services.Price as ServicePrice, Employee.FullName as EmployeeName, Companies.Name as CompanyName, \r\n\tCOUNT(*) OVER() AS TotalRows\tFROM ServiceHistory \r\n\tINNER JOIN Services ON Services.Id = ServiceHistory.ServiceId\r\nINNER JOIN Employee ON Employee.Id = ServiceHistory.E" +
"mployeeId\r\nINNER JOIN Companies ON Companies.Id = ServiceHistory.CompanyId\r\nWHERE CONVERT(date, DateOfCreate) >= CONVERT(date, @DateBegin) AND CONVERT(date, DateOfCreate) <= CONVERT(date, @DateEnd) \r\nAND ServiceHistory.CompanyId = @CompanyId AND ServiceId = @ServiceId)\r\n\r\nELSE IF @CompanyId > 0 AND @EmployeeId > 0\r\n(SELECT  Services.Name as ServiceName, Services.Price as ServicePrice, Employee.FullName as EmployeeName, Companies.Name as CompanyName, \r\n\tCOUNT(*) OVER() AS TotalRows\tFROM ServiceHistory \r\n\tINNER JOIN Services ON Services.Id = ServiceHistory.ServiceId\r\nINNER JOIN Employee ON Employee.Id = ServiceHistory.EmployeeId\r\nINNER JOIN Companies ON Companies.Id = ServiceHistory.CompanyId\r\nWHERE CONVERT(date, DateOfCreate) >= CONVERT(date, @DateBegin) AND CONVERT(date, DateOfCreate) <= CONVERT(date, @DateEnd)\r\nAND ServiceHistory.CompanyId = @CompanyId AND EmployeeId = @EmployeeId)\r\n\r\nELSE IF @EmployeeId > 0 AND @ServiceId > 0\r\n(SELECT  Services.Name as ServiceName, Services.Price as ServicePrice, Emp" +
"loyee.FullName as EmployeeName, Companies.Name as CompanyName, \r\n\tCOUNT(*) OVER() AS TotalRows\tFROM ServiceHistory \r\n\tINNER JOIN Services ON Services.Id = ServiceHistory.ServiceId\r\nINNER JOIN Employee ON Employee.Id = ServiceHistory.EmployeeId\r\nINNER JOIN Companies ON Companies.Id = ServiceHistory.CompanyId\r\nWHERE CONVERT(date, DateOfCreate) >= CONVERT(date, @DateBegin) AND CONVERT(date, DateOfCreate) <= CONVERT(date, @DateEnd) \r\nAND ServiceHistory.EmployeeId = @EmployeeId AND ServiceId = @ServiceId)\r\n\r\nELSE IF @CompanyId > 0 \r\n(SELECT  Services.Name as ServiceName, Services.Price as ServicePrice, Employee.FullName as EmployeeName, Companies.Name as CompanyName, \r\n\tCOUNT(*) OVER() AS TotalRows\tFROM ServiceHistory \r\n\tINNER JOIN Services ON Services.Id = ServiceHistory.ServiceId\r\nINNER JOIN Employee ON Employee.Id = ServiceHistory.EmployeeId\r\nINNER JOIN Companies ON Companies.Id = ServiceHistory.CompanyId\r\nWHERE CONVERT(date, DateOfCreate) >= CONVERT(date, @DateBegin) AND CONVERT(date, DateOfCreate) <=" +
" CONVERT(date, @DateEnd) \r\nAND ServiceHistory.CompanyId = @CompanyId)\r\n\r\nELSE IF @ServiceId > 0 \r\n(SELECT  Services.Name as ServiceName, Services.Price as ServicePrice, Employee.FullName as EmployeeName, Companies.Name as CompanyName, \r\n\tCOUNT(*) OVER() AS TotalRows\tFROM ServiceHistory \r\n\tINNER JOIN Services ON Services.Id = ServiceHistory.ServiceId\r\nINNER JOIN Employee ON Employee.Id = ServiceHistory.EmployeeId\r\nINNER JOIN Companies ON Companies.Id = ServiceHistory.CompanyId\r\nWHERE CONVERT(date, DateOfCreate) >= CONVERT(date, @DateBegin) AND CONVERT(date, DateOfCreate) <= CONVERT(date, @DateEnd)\r\nAND ServiceHistory.ServiceId = @ServiceId)\r\n\r\nELSE IF @EmployeeId > 0 \r\n(SELECT  Services.Name as ServiceName, Services.Price as ServicePrice, Employee.FullName as EmployeeName, Companies.Name as CompanyName, \r\n\tCOUNT(*) OVER() AS TotalRows\tFROM ServiceHistory \r\n\tINNER JOIN Services ON Services.Id = ServiceHistory.ServiceId\r\nINNER JOIN Employee ON Employee.Id = ServiceHistory.EmployeeId\r\nINNER JOIN Companie" +
"s ON Companies.Id = ServiceHistory.CompanyId\r\nWHERE CONVERT(date, DateOfCreate) >= CONVERT(date, @DateBegin) AND CONVERT(date, DateOfCreate) <= CONVERT(date, @DateEnd)\r\nAND ServiceHistory.EmployeeId = @EmployeeId)\r\n\r\nELSE\r\n(SELECT  Services.Name as ServiceName, Services.Price as ServicePrice, Employee.FullName as EmployeeName, Companies.Name as CompanyName, \r\n\tCOUNT(*) OVER() AS TotalRows\tFROM ServiceHistory \r\n\tINNER JOIN Services ON Services.Id = ServiceHistory.ServiceId\r\nINNER JOIN Employee ON Employee.Id = ServiceHistory.EmployeeId\r\nINNER JOIN Companies ON Companies.Id = ServiceHistory.CompanyId\r\nWHERE CONVERT(date, DateOfCreate) >= CONVERT(date, @DateBegin) AND CONVERT(date, DateOfCreate) <= CONVERT(date, @DateEnd))";
        }
